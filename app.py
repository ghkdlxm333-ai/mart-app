...
if error:
    ...
    if uploaded_file:
        if "ordview" in uploaded_file.name.lower():
            try:
                ...
                for _, row in df_raw.iterrows():
                    ...
                    final_rows.append({
                        ...
                        '발주처코드': '81020000',      # 무조건 고정
                        '발주처': '홈플러스',          # 무조건 고정
                        '배송코드': shipping_code,
                        '배송지': target_delivery_place,
                        '상품코드': p_info['me'],
                        '상품명': p_info['nm'],
                ...
                if not final_rows:
                    ...
                    st.success(f"성공적으로 {len(df_final)}건의 데이터를 변환했습니다.")
                    ...
                    st.download_button(
                        label="📥 홈플러스 수주양식 다운로드",
                        data=output.getvalue(),
                        file_name=f"Homeplus_Order_{datetime.now().strftime('%Y%m%d')}.xlsx",
            ...
            st.error("파일명에 'ordview'가 포함된 엑셀 파일을 업로드해주세요.")
